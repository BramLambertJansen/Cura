    ...
    <Sheet onClose={onClose} tall>
      ...
      <div className="flex gap-2 mb-7">
        <input value={naam} onChange={(e) => setNaam(e.target.value)} disabled={!editing}
          className={`flex-1 rounded-2xl px-4 py-3.5 text-foreground outline-none text-sm transition-all ${editing ? "border border-border/60" : ""}`}
          style={{ background: editing ? "var(--input-background)" : "var(--muted)", boxShadow: editing ? `0 0 0 2px color-mix(in srgb, var(--primary) 26%, transparent)` : "none" }} />
