        ...
        <div className="flex-1 min-w-0">
          {editing ? (
            <input value={naam} onChange={(e) => setNaam(e.target.value)}
              ...
              autoFocus
              aria-label="Naam"
              className="w-full rounded-2xl px-3.5 py-2 text-foreground outline-none text-base border border-border transition-all"
              style={{ background: "var(--input-background)", boxShadow: `var(--shadow-input), 0 0 0 2px color-mix(in srgb, var(--primary) 26%, transparent)` }} />
          ) : (
            <h3 className="text-xl font-medium text-foreground leading-tight font-display truncate">{weergaveNaam}</h3>
          )}
          <p className="text-sm text-muted-foreground mt-0.5 truncate">{household?.name ?? "Thuis"}</p>
        </div>
