        ...
        <div className="space-y-4 pb-20">
          ...
          {filteredTasks.length === 0 ? (
            ...
            <motion.div variants={stagger} initial="initial" animate="animate" className="space-y-3">
              {openTasks.map((t) => (
                <motion.div key={t.id} variants={fadeUp}>
                  <TaakRij task={t} onToggle={() => toggleTask(t.id, !t.done)} showClaim onClaim={() => claimTask(t.id, true)} onUnclaim={() => claimTask(t.id, false)} onEdit={() => openEditTask(t.id)} onStartFocus={() => startFocus(t)} onDismiss={() => dismissWithUndo(t, "alle taken")} />
                </motion.div>
              ))}
              {doneTasks.map((t) => (
                <motion.div key={t.id} variants={fadeUp}>
