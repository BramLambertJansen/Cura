...
export function BottomNav({ showAdd, onAdd }: { showAdd: boolean; onAdd: () => void }) {
  ...
  // Samen, Takenoverzicht, Focustimer, Boodschappen en AI-voorstellen hebben geen eigen tab (CLAUDE.md §1) — bereikbaar via Meer, dus die tab blijft actief.
  const isActive = (to: string) =>
    pathname.startsWith(to) ||
    (to === "/meer" && (pathname.startsWith("/samen") || pathname.startsWith("/taken") || pathname.startsWith("/focus") || pathname.startsWith("/boodschappen") || pathname.startsWith("/ai-voorstellen")));
