...
- Geen eigen navigatietab; bereikbaar via **Meer** (icoon `Timer`) en houdt de Meer-tab opgelicht via de `isActive`-special-case in `BottomNav` (zoals Samen/Taken). Vanaf een taak start je via een klein `Timer`-knopje op `TaakRij` (`onStartFocus`-prop, gedeeld hulp-hook `useStartFocus`, `src/app/lib/`) — bedraad op Vandaag/Taken/Huis.
