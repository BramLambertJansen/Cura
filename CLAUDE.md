...
- **Profiel** (`ProfielSheet`) — eigen weergavenaam bewerken (`updateMyDisplayName`, geeft door aan `DataStore.updateMember`), instellingen, meldingen-toggle (gekoppeld aan echte `Notification.permission` via `useNotificationPreference`), donkere-modus-toggle (`useDarkMode`, een `.dark`-class op `<html>` + localStorage, toegepast vóór de eerste render via `applyStoredDarkMode()` in `main.tsx`), uitloggen. Account/Help & feedback zijn bewust niet-klikbare "binnenkort"-rijen in plaats van stub-toasts.
