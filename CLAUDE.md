...
- Open taken staan in één gedeelde kaart, gegroepeerd in een dagdeel-tijdlijn (Ochtend/Middag/Avond, huidig dagdeel gemarkeerd met "nu") via `toDagdelen`/`dagdeelForHour` (`src/data/selectors.ts`) — alleen een taak met een echte wekker/`dueDate` krijgt een dagdeel (afgeleid van dat tijdstip); de rest valt in een neutrale "Overig"-groep in plaats van een verzonnen moment (§2: eerlijkheid boven precisie). Rijen zijn `TijdlijnTaakRij` (`src/app/components/TijdlijnTaakRij.tsx`) — de tijdlijn-variant van `TaakRij` (deelt alleen de swipe-drempel-constanten): geen eigen kaart-chrome, rijen staan direct in de gedeelde dagdeel-kaart; verder dezelfde interacties als `TaakRij` (veeg rechts = afvinken, veeg links = focustimer starten wanneer de taak timebaar is — anders "niet vandaag", tik op titel = bewerken, timer-knop).
- Afgeronde geplande taken staan niet tussen de open taken, maar in een aparte, standaard ingeklapte "N afgerond vandaag"-sectie (telbadge + chevron); terugzetten kan met één tik op de rij.
...
- De oude, altijd-zichtbare huisgenoot-activiteitsstrook bovenaan is vervangen door een inklapbare "Logboek"-sectie onderaan (telbadge + chevron, standaard dicht) met zowel je eigen als je huisgenoot's afgeronde taken van vandaag, nieuwste eerst — nog steeds puur een bericht, geen log met tellingen per persoon.
...
- Bovenaan een `Kamers`/`Alle taken`-tabtoggle (`role="tablist"`, lokale component-state). `Kamers` is de kamerlijst hieronder; `Alle taken` toont alle open + afgeronde taken huishoud-breed via `TaakRij`, filterbaar op kamer en duur (`KeuzeChip`-pills: alle/≤15/15–45/45+ min) — puur een client-side filter over `useTaskViews()`, geen eigen view-model/selector.
...
- Geen eigen navigatietab; bereikbaar via **Meer** (icoon `Timer`) en houdt de Meer-tab opgelicht via de `isActive`-special-case in `BottomNav` (zoals Samen/Taken). Vanaf een taak start je via een klein `Timer`-knopje op `TaakRij` (`onStartFocus`-prop, gedeeld hulp-hook `useStartFocus`, `src/app/lib/`) — bedraad op Vandaag/Taken/Huis — óf door de rij naar links te vegen (zie §Swipe & verversen).
...
- Snel toevoegen via een pagina-lokale `BoodschapToevoegRij` (aantal + titel, twee velden) bovenaan de pagina — geen sheet; rijen (`BoodschapRij`, `src/app/components/`) hebben geen swipe/tik-om-te-bewerken zoals `TaakRij`, en losse items verwijderen heeft bewust géén bevestigingsstap (laag risico, triviaal opnieuw toe te voegen).
...
- Migratie `20260706040000_shopping_items.sql` (RLS + realtime-publicatie, zelfde patroon als `rooms`).
...
### Swipe & verversen
- `TaakRij` ondersteunt veeg-naar-rechts om af te vinken/terug te zetten (Framer `drag="x"`, sage check-cirkel onthult zich achter de kaart; puur een verrijking — de checkbox blijft de toetsenbord/screenreader-route, en `useReducedMotion` schakelt het slepen uit). Veeg-naar-links start de **focustimer** voor de taak wanneer die timebaar is (open taak met `onStartFocus` bedraad — sage timer-cirkel onthult zich rechts); een rij die niet timebaar is maar wél `onDismiss` heeft, houdt veeg-links als "niet vandaag" (rode ×). De timer wint dus van dismiss waar een taak echt getimed kan worden; de swipe blijft een verrijking bovenop de zichtbare timer-knop. `TijdlijnTaakRij` spiegelt dit één-op-één.
...
- `CreateHouseholdPage`: "noem je huishouden" voor een ingelogde gebruiker zonder huishouden, na de intro.
